...
export function cleanCvText(input = '') {
  ...
  s = s
    ...
    .replace(/([a-z0-9._%+-]+)@([a-z0-9._%+-]+)\s+([a-z]{2,})\b/gi, '$1@$2.$3')
    .replace(/[ \t]+([,.;:!?])/g, '$1')
    .replace(/\s+,/g, ',')
    .replace(/,{2,}/g, ',');
...
function extractPortfolio(text) {
  ...
  if (/portfolio|instagram\.com|dribbble\.com|github\.com/i.test(text)) parts.push('Portfolio');
  return parts.join(' · ');
}
...
export function extractionQuality(cv = {}, rawText = '') {
...
export function getCleanSampleCv(jobHint = '') {
...
export function normalizeExtraction(rawText = '', jobHint = '') {
...
function parseCvTextCore(text, jobHint, sections, lines) {
  const clients = extractClients(text);
  const name = inferName(lines, text) || '';
  const title = inferTitle(lines, text, jobHint) || '';
...
/** Sub-scores from structured cvData (premiumCV / cvDraft), not random placeholders. */
export function scoreFromCvData(cv = {}, job = '', rawText = '') {
  const c = sanitizePremiumCV(cv);
...
export function buildRecruiterInsights(scores = {}, cv = {}) {
  const c = sanitizePremiumCV(cv);
  ...
  if ((c.clients || []).length >= 2 && im < 72) {
    add(weaknesses, 'Strong client credibility, but proof appears too low.');
  }
...
export function buildRecruiterFixes(scores = {}, cv = {}, job = '') {
  const fixes = [];
  const c = sanitizePremiumCV(cv);
...
export function buildRecruiterVerdict(scores = {}) {
