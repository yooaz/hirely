...
console.log(JSON.stringify({
  ...
  metadata: {
    contact: res.structured?.metadata?.contact,
    title: res.structured?.identity?.title,
    warnings: res.structured?.metadata?.warnings,
  },
  topUnsorted: res.structured?.unsorted?.slice(0,5)
},null,2));
